...
public class ConfigWindow : Window, IDisposable
{
    ...
    public override void Draw()
    {
        ...
        {
            var github_issues_url = "https://github.com/FranFkntastic/ComplicatedMarketBoard/issues";
            ...
            ImGui.Indent();
            ImGui.Text("Goto: plugin-help-forum > Complicated Market Board");
            ImGui.Unindent();
            ImGui.Text("2) PM @franfkntastic");
            ImGui.Unindent();
...
// #endif
    ...
    private void DrawGeneral(float padding)
    {
        ...
        if (ImGui.Button("Manage custom scopes"))
        ...
        {
            foreach (var type in Enum.GetValues<PriceDisplayMode>())
            {
    ...
    {
        ScopeTreeEditor.Draw(P.Config.AdditionalWorlds, suffix, true, false, "", () =>
        {
            P.Config.Save();
            P.MainWindow.UpdateWorld();
        });
        ScopeTreeEditor.DrawUnknownSavedEntries(P.Config.AdditionalWorlds, suffix, () =>
        {
            P.Config.Save();
            P.MainWindow.UpdateWorld();
        });
    ...
    private void DrawUi(float padding)
    {
        ...
        ImGuiComponents.HelpMarker(
            "For sold table, the new position Y' = Y + offset. Drag the divider between selling and sold tables to update this value."
        );
        ...
        ImGui.BeginChild("table DrawUi Position Offset", new Vector2(table_width, table_height * 11), false);
        ImGui.Columns(2);
        ImGui.SetColumnWidth(0, col_name_width);
        ImGui.SetColumnWidth(1, col_value_width);
        ...
        ImGui.NextColumn();
        if (P.Config.sellingColWidthOffset.Length < 5) Array.Resize(ref P.Config.sellingColWidthOffset, 5);
        ...
        {
            var sellingColWidthOffsetValues = sellingColWidthOffset.ToArray();
            for (var i = 0; i < sellingColWidthOffsetValues.Length; i++) P.Config.sellingColWidthOffset[i] = sellingColWidthOffsetValues[i];
            P.Config.Save();
        }
        ImGuiComponents.HelpMarker("Offset the width of each column in the selling table. Drag column borders in the main window to update these values.");
        ImGui.NextColumn();

        ImGui.Text("Retainer column width");
        ImGui.NextColumn();
        var retainerColWidthOffset = P.Config.sellingColWidthOffset[4];
        ImGui.SetNextItemWidth(col_value_content_width);
        if (ImGui.InputFloat($"{suffix}retainerColWidthOffset", ref retainerColWidthOffset, 0.0f, 0.0f, "%.0f"))
        {
            P.Config.sellingColWidthOffset[4] = retainerColWidthOffset;
            P.Config.Save();
        }
        ImGuiComponents.HelpMarker("Offset the width of the retainer column in the selling table. Drag column borders in the main window to update this value.");
        ImGui.NextColumn();
        ...
        ImGui.NextColumn();
        if (P.Config.soldColWidthOffset.Length < 5) Array.Resize(ref P.Config.soldColWidthOffset, 5);
        ...
        {
            var soldColWidthOffsetValues = soldColWidthOffset.ToArray();
            for (var i = 0; i < soldColWidthOffsetValues.Length; i++) P.Config.soldColWidthOffset[i] = soldColWidthOffsetValues[i];
            P.Config.Save();
        }
        ImGuiComponents.HelpMarker("Offset the width of each column in the sold table. Drag column borders in the main window to update these values.");
        ImGui.NextColumn();

        ImGui.Text("Buyer column width");
        ImGui.NextColumn();
        var buyerColWidthOffset = P.Config.soldColWidthOffset[4];
        ImGui.SetNextItemWidth(col_value_content_width);
        if (ImGui.InputFloat($"{suffix}buyerColWidthOffset", ref buyerColWidthOffset, 0.0f, 0.0f, "%.0f"))
        {
            P.Config.soldColWidthOffset[4] = buyerColWidthOffset;
            P.Config.Save();
        }
        ImGuiComponents.HelpMarker("Offset the width of the buyer column in the sold table. Drag column borders in the main window to update this value.");
        ImGui.NextColumn();
        ...
        ImGuiComponents.HelpMarker("The width of the panel on the right of the main window. Drag the divider between the main tables and right panel to update this value.");
        ImGui.NextColumn();

        // WorldUpdateTableHeight
        ImGui.Text("World Updated height");
        ImGui.NextColumn();
        var WorldUpdateTableHeight = P.Config.WorldUpdateTableHeight;
        ImGui.SetNextItemWidth(col_value_content_width);
        if (ImGui.InputFloat($"{suffix}WorldUpdateTableHeight", ref WorldUpdateTableHeight, 0.0f, 0.0f, "%.0f"))
        {
            if (WorldUpdateTableHeight < 60) WorldUpdateTableHeight = 60;
            P.Config.WorldUpdateTableHeight = WorldUpdateTableHeight;
            P.Config.Save();
        }
        ImGuiComponents.HelpMarker("The height reserved for the world freshness table on the right bottom. Drag the divider above the table to update this value.");
        ImGui.NextColumn();
        ...
        ImGuiComponents.HelpMarker("The width of the columns of the last updated time for each world on the right bottom. Drag column borders in the main window to update these values.");
        ImGui.NextColumn();
